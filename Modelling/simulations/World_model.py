...
Simulate performance of world-centered localization in which the probability of making a west-ward response is shaped by:
    - sound angle in the world (sometimes called the 'Full Allocentric' simulation)
...
def main():
    ...
    df['response_activation'] = csim.run_mdl(df['speaker_angle_world'], coefs_world)
    ...
    csim.plot_pCorrect(ax, world_out, 'k')             
    ...
    #######################################################################################################################     
    plt.savefig('Modelling/images/jneurosci/World_model.png', dpi=600)
    # plt.show()
