function TestModel_CF8_FullAllo_Theta_FixedParams
...
plot_stim_vs_pSpout9(nSim, X_chosen, stim, response, ax, true)
...
plot_stim_vs_pSpout9(nSim, X_chosen, stim, response, ax, false)
...
for i = 1 : nSim
    ...
    if create_stim
        ...
        [sim_resp, sim_stim] = simulate_CF8_FullAllo_Theta( nTrials, X(1), X(2), X(3), true_stim);
    end
...
correct_stim = test_data(:, strcmp( header, 'Speaker Location'));
correct_resp = test_data(:, strcmp( header, 'Response'));
correct_theta_d = test_data(:, strcmp( header, 'speaker_angle'));
