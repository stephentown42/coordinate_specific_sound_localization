function TestModel_CF10_FullEgo_Theta
...
for i = 1 : nSim
    ...
    if create_stim
        [sim_resp, sim_stim] = simulate_CF9_PartEgo_Theta( nTrials, X(1), X(2), X(3));
    else
        [sim_resp, sim_stim] = simulate_CF9_PartEgo_Theta( nTrials, X(1), X(2), X(3), true_stim);
    end
    ...
    [world_stim_sim, p9_byWorld_sim(:,i)] = get_pLeft_byStimVal(sim_resp, sim_stim.theta_d);    
    [head_stim_sim,  p9_byHead_sim(:,i)]  = get_pLeft_byStimVal(sim_resp, sim_stim.head_stim_angle);    
...
% Get ferret performance
[world_stim_true, p9_byWorld_true] = get_pLeft_byStimVal(true_response, true_stim.theta_d);      
[head_stim_true,  p9_byHead_true]  = get_pLeft_byStimVal(true_response, true_stim.head_stim_angle);
[true_angles, tru_pCorrect] = get_pCorrect_by_center_angle(true_stim, true_response);
...
h = plot( world_stim_true, p9_byWorld_true,...
            'color',[0 0.4 0.8],...
            'LineWidth', 2,...
            'Marker','.',...
            'MarkerSize',12);
...
h = plot( head_stim_true, p9_byHead_true,...
            'color',[0 0.4 0.8],...
            'LineWidth', 2,...
            'Marker','.',...
            'MarkerSize',12);
...
h = plot(true_angles, tru_pCorrect,...
            'color',[0 0.4 0.8],...
            'LineWidth', 2,...
            'Marker','.',...
            'MarkerSize',12);
...
function [uniq_x, p9] = get_pLeft_byStimVal(response, stim)
%
...
response = data(:, strcmp(header, 'response_angle_platform'));
