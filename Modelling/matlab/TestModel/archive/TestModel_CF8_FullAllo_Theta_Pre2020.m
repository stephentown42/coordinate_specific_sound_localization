...
for i = 1 : nFolds  % For each fold
    ...
    [Xfit(i,:), NLL(i)] = fit_data_multiple_runs(nFit, train_folds, 'theta_d', ax);
    
    fold_performance(i) = test_model_performance( test_fold, Xfit(i,:));
end

% Plot cross validation performance
axes('position', [0.85 0.76 0.1 0.2]);       
title('Predicting behavior')
boxANDscatter(1, fold_performance, 'k', 'b')
plotXLine(50)
ylabel('% Correct')
box off
set(gca,'xcolor','none')
...
[predicted_response, ~] = simulate_CF8_FullAllo_Theta(0, X, test_fold);
...
[Xfit, X0, bestX] = deal(nan(nRuns, 3));
NegLL = nan(nRuns, 1);
...
speaker = data(:, col.Speaker);
not_probe = speaker == 6 | speaker == 12;
...
all_data = table(speaker, response, theta_d, rho, x, y);
all_data.Correct = data(:, col.Correct);
all_data.center_spout_angle = data(:, col.SpoutRotation);
all_data.head_stim_angle = data(:, col.HeadStimAngle);

test_data = all_data(not_probe, :);

% Get response contingency from test data
correct_data = test_data(test_data.Correct == 1, {'speaker','response','theta_d'});
