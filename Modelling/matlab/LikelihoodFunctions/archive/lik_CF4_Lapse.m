function NegLL = lik_CF4_Lapse(response, stim, bias, lapse_rate)
%
%
% response, stim is the data that model fit is assessed for
% bias, lapse_rate are the model parameter being fitted
%
...
for i = 1 : nStim
    ...
    p_response( trial_idx, act_idx) = 1-lapse_rate;
    p_response( trial_idx, ~act_idx) = lapse_rate / (nActions-1);    
end
...
for trial = 1 : nTrials
    ...
    trial_p = exp(Q) / sum(exp(Q));   % softmax
