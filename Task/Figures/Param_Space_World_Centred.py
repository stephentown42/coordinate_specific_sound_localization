...
def main():
    ...
    fig_size = (4,4)    # In inches, - make this big, I don't care how big as long as it can fit the figure in
    ...
    sound_angle_head = np.arange(-180, 210, 30) 

    # Plot probe speakers
    for probe_angle in range(30, 180, 30):
        sound_angle_world = np.full_like(sound_angle_head, probe_angle)
   ...
   # Plot platform angles
    ...
    # Plot north speaker
    sound_angle_world = np.full_like(sound_angle_head, 180)

    ax.plot(sound_angle_head, sound_angle_world, c='#faa600', linestyle='-', marker='.')
    ax.plot(sound_angle_head, -sound_angle_world, c='#faa600', linestyle='-', marker='.')

    # Plot south speaker
    sound_angle_world = np.full_like(sound_angle_head, 0)

    ax.plot(sound_angle_head, sound_angle_world, c='#004d40', linestyle='-', marker='.')
    ...
    # plt.show()
    plt.savefig('Task/Figures/ParamSpace_WorldCentred.png', dpi=600)
